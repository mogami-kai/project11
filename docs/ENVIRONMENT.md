...
6. **ops_rules Normative Override**: `ADMIN_ALLOWED_IPS` is normative per `docs/docs_ops_rules.md §6`. Its absence is a deployment BLOCK regardless of any other gate status.
...
| `ADMIN_ALLOWED_IPS` | variable | No | **Yes** (BLOCK) | No | `wrangler.toml:10`, `access.js:71`, `admin.js:244`, `docs_ops_rules.md §6` | Comma-separated IPv4 allowlist for admin routes |
...
| `LIFF_HOTEL_URL` | variable | No | **Yes** | Conditional | `wrangler.toml:18`, `broadcastMessage.js:16` | LIFF URL for hotel intent (broadcast messages) |
...
### 4.7 LIFF URLs
`LIFF_URL` acts as a general fallback when `LIFF_TRAFFIC_URL`, `LIFF_EXPENSE_URL`, or `LIFF_HOTEL_URL` are not set. For broadcast messages, each specific URL is preferred. All four should be set in production for correct broadcast button behavior.
...
- [ ] `ADMIN_ALLOWED_IPS` — **non-empty**, contains ≥1 valid IPv4 address (**BLOCK if absent**)
...
|---|---|---|
| `ADMIN_ALLOWED_IPS` absent or empty | **BLOCK** | Normative per `docs_ops_rules.md §6`; admin routes lack IP protection |
...
| `ADMIN_ALLOWED_IPS` | IP change | `wrangler deploy` after updating `wrangler.toml [vars]` |
...
### ADMIN_ALLOWED_IPS Missing

1. This is a **BLOCK** condition. Do not proceed with deployment.
2. Determine correct IP(s) for production admin callers.
3. Set `ADMIN_ALLOWED_IPS` in `wrangler.toml [vars]` and redeploy.
4. Verify `/api/admin/shift/raw/recent` is IP-gated before marking incident resolved.
...
>
> `ADMIN_ALLOWED_IPS` is additionally normative per `docs/docs_ops_rules.md §6`. Its absence constitutes a specification violation independent of all other checks.
>
